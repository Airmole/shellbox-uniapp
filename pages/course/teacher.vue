			...
			<view class="margin">
				<view :class="'cu-list menu sm-border '+(foldOptionsArea?'round':'card-radius')">
					...
					<template v-if="!foldOptionsArea">
						...
						<view class="cu-item">
							<view class="content">
								<button v-if="teacherCourses !== ''" @click="exportXlsx" class="cu-btn round bg-gradual-blue shadow"><text
		...
		methods: {
			...
			fetchTeacherCourse () {
				...
				api.fetchTeacherCourse(
					...
					videoAd = initalVideoAd(api.exportTeacherCourse, res.data, '导出教师课表')
					uni.hideLoading()
				}).catch((res) => {
